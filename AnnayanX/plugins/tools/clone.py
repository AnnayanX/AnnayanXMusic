...
from AnnayanX.utils import get_assistant, clonebotdb
    ...
    if len(message.command) > 1:
        ...
        try:
            ai = Client(
                ...
                API_HASH,
                bot_token=bot_token,
                plugins=dict(root="AnnayanX.cplugin"),
            )
            await ai.start()
            bot = await ai.get_me()
        ...
        try:
            ...
            await mi.edit_text(
                f"⚠️ <b>ᴇʀʀᴏʀ:</b>\n\n<code>{e}</code>\n\n**ᴋɪɴᴅʟʏ ғᴏᴡᴀʀᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ @greatvasudevkrishna ᴛᴏ ɢᴇᴛ ᴀssɪsᴛᴀɴᴄᴇ**"
            )
    else:
